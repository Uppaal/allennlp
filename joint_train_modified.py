...
def process(args):
    ...
    for split in splits:
        ...
        if split == "dev":
            ...
            rest_output_data = []
            rest_output_version = []
            rest_output_data.extend(tf['data'][t_len:len(tf['data'])])
            rest_output_version.extend(tf['version'][t_len:len(tf['data'])])
            rest_of = {"data": rest_output_data, "version": rest_output_version}
            with open(output_dir + "rest-v1.1.json",'w') as fp:
                json.dump(rest_of, fp)
