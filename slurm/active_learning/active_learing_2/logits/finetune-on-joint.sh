...
#
#SBATCH --mem=50000
#SBATCH --job-name=a-c1-ft-j
#SBATCH --partition=m40-long
#SBATCH --output=classifier-top1-finetune-on-joint-%A.out
#SBATCH --error=classifier-top1-finetune-on-joint-%A.err
#SBATCH --gres=gpu:1
...
python -m allennlp.run fine-tune \
    ...
    --serialization-dir output/active_classifier_top_1_fn-joint

# Evaluate on SQuAD
python -m allennlp.run evaluate output/active_classifier_top_1_fn-joint/model.tar.gz --evaluation-data-file "data/squad/dev-v1.1.json"

# Evaluate on NewsQA
python -m allennlp.run evaluate output/active_classifier_top_1_fn-joint/model.tar.gz --evaluation-data-file "data/NewsQA/test-v1.1.json"
